...
class YOLO :public QObject
{
	Q_OBJECT
public:
	void path(std::string engine_file_path);
	virtual ~YOLO();
	void detect_img(cv::Mat& image);
	void detect_video(cv::Mat& frame);
	cv::Mat static_resize(cv::Mat& img);
	float* blobFromImage(cv::Mat& img);
	void doInference(IExecutionContext& context, float* input, float* output, const int output_size, cv::Size input_shape);
	QString msg;
	int count_car = 0, count_person = 0;


private:
	static const int INPUT_W = 640;
	static const int INPUT_H = 640;
	const char* INPUT_BLOB_NAME = "images";//修改输入
	const char* OUTPUT_BLOB_NAME = "output";//修改输出
	float* prob;
	int output_size = 1;
	ICudaEngine* engine;
	IRuntime* runtime;
...
void YOLO::path(std::string engine_file_path)
{
	size_t size{ 0 };
...
void YOLO::detect_img(cv::Mat& image)
{
	int img_w = image.cols;
	int img_h = image.rows;
	cv::Mat pr_img = this->static_resize(image);
	std::cout << "blob image" << std::endl;

	float* blob;
	blob = blobFromImage(pr_img);
	float scale = std::min(this->INPUT_W / (image.cols * 1.0), this->INPUT_H / (image.rows * 1.0));
	...
	for (size_t i = 0; i < objects.size(); i++)
	{
		...
		if (class_names[obj.label] == "car")
		{
			count_car = count_car + 1;
		}
		else if (class_names[obj.label] == "person")
		{
			count_person = count_person + 1;
		}
		msg = QString("total number:%1\ncar number:%2\nperson number:%3\n").arg(objects.size()).arg(count_car).arg(count_person);
...
void YOLO::detect_video(cv::Mat& frame)
{
	int img_w = frame.cols;
	int img_h = frame.rows;
	cv::Mat pr_img = this->static_resize(frame);

	float* blob;
	blob = blobFromImage(pr_img);
	float scale = std::min(this->INPUT_W / (frame.cols * 1.0), this->INPUT_H / (frame.rows * 1.0));
	...
	cv::putText(frame, fpsString, cv::Point(5, 20), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
	...
	for (size_t i = 0; i < objects.size(); i++)
	{
		...
		cv::rectangle(frame, obj.rect, color * 255, 2);
